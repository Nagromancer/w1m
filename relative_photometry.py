...
dates = ["20250205", "20250206", "20250207", "20250208", "20250213", "20250216", "20250218", "20250219", "20250220",
         "20250221", "20250222", "20250223", "20250224", "20250225", "20250226", "20250310", "20250317", "20250318",
         "20250320", "20250321",
         # "20250322", "20250324", "20250325", "20250326", "20250327", "20250404", "20250406", "20250413", "20250415",
         "20250417", "20250418", "20250419", "20250420", "20250422", "20250423", "20250424", "20250425", "20250426",
         "20250427", "20250503", "20250504", "20250514", "20250515", "20250516", "20250518", "20250520", "20250528"]
...
for date in dates:
    try:
        ...
        phot_path = Path(
            f"/Volumes/SanDisk-2TB-SSD/w1m/dates/{date}/{camera}/{target}/{date}-{target}-{camera}-phot.fits")
    ...
    flattened, trend = wotan.flatten(time=times, flux=relative_flux, method='biweight', window_length=0.02,
                                     return_trend=True)
    ...
    print(
        f"{target_mag:.2f} mag: Standard deviation: Trend - {trend_std_dev:.3f} ppt | Relative - {std_dev:.3f} ppt | Binned - {binned_std_dev:.3f} ppt")
...
# plt.errorbar(total_times - 2460000, total_fluxes, yerr=total_flux_errs, fmt='o', color='black', markersize=5, alpha=0.1)
plt.errorbar(total_binned_times - 2460000, total_binned_fluxes, yerr=total_binned_flux_errs, fmt='o', color='black',
             markersize=5)
# plt.plot(times, trend, color='red', lw=2)
plt.title(f"Target {target_id} ({target_mag:.2f} mag) - {aperture_radius} px")
plt.xlabel(f"Time (BJD - 2460000)")
...
combined_lc = Table()
combined_lc["Time_BJD"] = total_times
combined_lc["Flux"] = total_fluxes
combined_lc["Error"] = total_flux_errs
combined_lc.write(f"/Users/nagro/PycharmProjects/w1m/lcs/combined_lc_{aperture_radius}.csv", overwrite=True)
print(f"Saved combined light curve to combined_lc_{aperture_radius}.csv")
...
plt.errorbar((folded_binned_time % 1) * peak_period * 24, total_binned_fluxes + offset * np.floor(folded_binned_time),
             yerr=total_binned_flux_errs, fmt='o', color='black', markersize=5)
...
plt.scatter(cat[cat["VALID"]]["BP_MAG" if camera == "blue" else "RP_MAG"], average_fluxes, 40,
            c=cat[cat["VALID"]]["BP_RP"], cmap='coolwarm', clim=(0.0, 2.5))
# colorbar with label
...
x = np.linspace(np.min(cat[cat["VALID"]]["BP_MAG" if camera == "blue" else "RP_MAG"]),
                np.max(cat[cat["VALID"]]["BP_MAG" if camera == "blue" else "RP_MAG"]), 100)
