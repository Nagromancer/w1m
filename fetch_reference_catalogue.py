...
def query_tap(out_path, center, width, height, calibration_limits, blend_delta, step_distance=1 * u.degree):

    # store in the same directory as the out_path but with modified name
    gaia_result_path = out_path.dirname() / f"{out_path.basename()[9:-5]}.pkl"
    if not gaia_result_path.exists():
        ...
        pickle.dump(results, open(gaia_result_path, 'wb'))
    else:
        results = pickle.load(open(gaia_result_path, 'rb'))
...
def fetch_gaia(target, ra_center, dec_center, box_width, box_height, img_path, reference_epoch, output_path,
    ...
    catalog = query_tap(output_path, center, box_width * u.degree, box_height * u.degree, calibration_limits, blend_delta)
    ...
    output.sort('G_MAG')
    # output.pprint_all(max_width=-1, max_lines=-1)
...
if __name__ == "__main__":
    ...
    fetch_gaia(args.target, args.ra, args.dec, args.box_width, args.box_height, args.img_path, args.epoch, args.output,
