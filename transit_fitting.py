...
if obs == "tnt":
    initial_poses = [(0.0756, 1.0056, -0.0237, 0.0004, 0.0015, 0.1169, 0.0015, 0.0028, 0.0067, 0.3038),
                     (
                         -0.047, 1.1013, 1.8484, 0.0004, 0.0157, 0.1562, 1.9611, 0.0012, 0.0042, 0.2215, 1.9843, 0.0005,
                         0.0016, 0.1608, 2.0197, 0.0003, 0.0064, 0.1058),
                     ...
                     (-0.0471, 2.3194, 27.7706, 0.0055, 0.0035, 0.2338, 27.8737, 0.0044, 0.0029, 0.1993, 27.9965,
                      0.0011, 0.0015, 0.1539),
    ...
    initial_poses = [(-0.0417, 1.097, 2.1717, 0.0007, 0.008, 0.205),
                     ...
                     (0.0148, 0.7184, 19.1299, 0.0024, 0.0044, 0.3223, 19.2349, 0.0006, 0.0033, 0.2684, 19.3612,
                      0.0007, 0.0038, 0.1446),
...
for night, initial_pos in [(i, initial_poses[i]) for i in range(len(initial_poses))][-1:]:
    print(night, initial_pos)
    ...
    for i in range((num_parameters - 2) // 4):
        ...
        print(f"{area:.6f},{area_lower:.6f},{area_upper:.6f}", end=",")
    ...
    plt.plot(times + ephemeris - 2460000, model_function((0, 1, *best_fits[2:]), times), 'b-', label="Model")
    plt.errorbar(times + ephemeris - 2460000, fluxes / (best_fits[0] * times + best_fits[1]), yerr=flux_err / (best_fits[0] * times + best_fits[1]), fmt='o', color=colours[1], label="Data", alpha=0.5)
    plt.xlabel("BJD - 2 460 000")
    plt.ylabel("Flux")
    plt.grid()
    # plot vlines at the t_mins
    for t_min in night_t_mins:
        plt.axvline(t_min + ephemeris - 2460000, color='red', linestyle='--', alpha=1)
        plt.text(t_min + ephemeris - 2460000, 1.02, f"{t_min + ephemeris:.5f}", rotation=45, fontsize=16, color='red', ha='center')
    ylims = plt.gca().get_ylim()
    plt.xlim(824.484, 824.5)
    plt.show()

    plt.errorbar(times + ephemeris - 2460000,fluxes / (best_fits[0] * times + best_fits[1]) - model_function((0, 1, *best_fits[2:]), times), yerr=flux_err / (best_fits[0] * times + best_fits[1]), fmt='o', color=colours[1], label="Data", alpha=0.5)
    plt.xlabel("BJD - 2 460 000")
