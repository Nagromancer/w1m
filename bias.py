...
parser.add_argument('--overwrite', action='store_true', help='Overwrite existing master bias image.')
...
bias_files = raw_dir.files('*bias*')
bias_files = [file for file in bias_files if not "._" in str(file)]  # remove hidden files
if len(bias_files) == 0:
    print("No bias images found.")
    exit(1)
bias_files.sort()
colour = "blue" if "BLUE" in str(bias_files[0]) else "red" if "RED" in str(bias_files[0]) else raiseExceptions("Unknown colour")
...
# perform a trimmed mean to remove outliers - ignore 10% of the highest and lowest values
stacked_biases = np.mean(np.sort(stacked_biases, axis=2)[:, :, 1:-1], axis=2)
